...
static int get_part_info(char *part_name, fwfs_partition_t *part_info)
{
    u64 part_offset;
    u64 part_size;
    ...
    nand = &nand_info[0];
    part_info->u32_PageSize = nand->writesize;
    part_info->u32_BlkSize = nand->erasesize;
#else
    ...
    part_info->u32_PageSize = spinor_flash->page_size;
    part_info->u32_BlkSize = spinor_flash->sector_size;
#endif
    //FIXME: the flash layer should report right values
    part_info->u32_BlkSize = FIRMWAREFS_BLK_SIZE;
    part_info->u32_PageSize = 2048;

    if (!get_mtdpart_range(part_name, &part_offset, &part_size))
    {
        printf("get_mtdpart_range return error\n");
        return -1;
    }
    part_info->u16_BlkCnt = ((u32)part_size)/part_info->u32_BlkSize;
    part_info->u16_StartBlk = ((u32)part_offset)/part_info->u32_BlkSize;
...
S32  firmwarefs_mount(char *partition, char *mnt_path)
{
    ...
    err = get_part_info(partition, &_gFwfsMntHandle.cfg.fwfs_partition);
    if (err < 0)
        return err;
