...
struct sstar_pwm_save {
    int savepad[PWM_NUM];
    int saveduty[PWM_NUM];
    int saveperiod[PWM_NUM];
    int savediv[PWM_NUM];
    int base;
};
struct sstar_pwm_save pwmRCVR;
#endif
...
#ifdef CONFIG_PM_SLEEP
    pwmRCVR.saveduty[u8Id]=u32Duty;
    pwmRCVR.saveperiod[u8Id]=u32Period;
    pwmRCVR.savediv[u8Id]=u16Div;
    pwmRCVR.base=(u32)ms_chip->base;
#endif
...
#ifdef CONFIG_PM_SLEEP
void DrvPWMSuspend(int u8Id)
{
    if(pwmRCVR.base)
        SETREG16(pwmRCVR.base + u16REG_SW_RESET, 1<<u8Id);
}

void DrvPWMResume(int u8Id)
{
    printk("%s id:%d div:%d period:%x duty:%x \r\n", __FUNCTION__, u8Id, pwmRCVR.savediv[u8Id],pwmRCVR.saveperiod[u8Id],pwmRCVR.saveduty[u8Id]);

    if(pwmRCVR.base){

        OUTREG16(pwmRCVR.base+ (u8Id*0x80) + u16REG_PWM_DIV, (pwmRCVR.savediv[u8Id] & 0xFFFF));
        OUTREG16(pwmRCVR.base + (u8Id*0x80) + u16REG_PWM_PERIOD_L, (pwmRCVR.saveperiod[u8Id]&0xFFFF));
        OUTREG16(pwmRCVR.base + (u8Id*0x80) + u16REG_PWM_PERIOD_H, ((pwmRCVR.saveperiod[u8Id]>>16)&0x3));
        OUTREG16(pwmRCVR.base + (u8Id*0x80) + u16REG_PWM_DUTY_L, (pwmRCVR.saveduty[u8Id]&0xFFFF));
        OUTREG16(pwmRCVR.base + (u8Id*0x80) + u16REG_PWM_DUTY_H, ((pwmRCVR.saveduty[u8Id]>>16)&0x3));

         if(pwmRCVR.savepad[u8Id])
            DrvPWMPadSet(u8Id, (U8)pwmRCVR.savepad[u8Id]);
    }
}
#endif
