...
void _BootDispCtrl(DispOutCfg *pstDispOut, DispOutAspectRatio *pstAspectRatio, u32 u32Shift)
{
    ...
    {
        u32Interface = MHAL_DISP_INTF_TTL;
    }
    else
    {
...
#if defined(CONFIG_SSTAR_PNL)
    if(u32Interface == MHAL_DISP_INTF_TTL)
    {
    ...
    if (pstDispOut->pPQbuffer)
    {
        MHAL_DISP_PqConfig_t stHalPqCfg;
        memset(&stHalPqCfg, 0, sizeof(stHalPqCfg));
        stHalPqCfg.pData = pstDispOut->pPQbuffer;
        stHalPqCfg.u32DataSize = pstDispOut->u32PQBufferSize;
        BOOTLOGO_DBG(BOOTLOGO_DBG_LEVEL_INFO, "%s %d, pData: %p u32DataSize: %d\n",
            __FUNCTION__, __LINE__, stHalPqCfg.pData, stHalPqCfg.u32DataSize);
        MHAL_DISP_DeviceSetPqConfig(pDevCtx, &stHalPqCfg);
    }
    ...
    if(u32Interface == MHAL_DISP_INTF_TTL)
    {
        if (pPnlDev == NULL)
        {
            ...
            if (pstDispOut->pstPnlParaCfg)
            {
                ...
                else if(pstDispOut->pstPnlParaCfg->eLinkType == E_MHAL_PNL_LINK_TTL_SPI_IF)
                {
                    Lcm_init((unsigned char *)pstDispOut->pstMipiDsiCfg->pu8CmdBuf, pstDispOut->pstMipiDsiCfg->u32CmdBufSize);
                }
            }
            else if (pstDispOut->pstUniPnlParaCfg)
            {
...
static int _MiscPartiton(u32 u32LogoId, DispOutAspectRatio *pstAspectRatio, LogoRotation_e eRot)
{
    ...
    switch (stDispOut.enDev)
    {
        ...
        case EN_DISPLAY_DEVICE_LCD:
            ...
            INI_GET_PNL_INT(stPnlPara.u16FDclk, pTargetPnlName, "m_wPanelFixedDCLK");
            if(stPnlPara.u16FDclk)
            ...
            INI_GET_PNL_INT(s32TtlCusInitTemp, pTargetPnlName, "m_ePanelPwBlCtrl");
            if(s32TtlCusInitTemp > 0)
                stPnlPara.u8TtlCstInitFlag = 1;
            INI_GET_PNL_INT(stPnlPara.stTtlCstmInitInfo.u16PwGpioNum, pTargetPnlName, "m_wPanelPwGpioNum");
            INI_GET_PNL_INT(stPnlPara.stTtlCstmInitInfo.u32DlyTmData2Bl, pTargetPnlName, "m_wPanelDlyTmData2Bl");
            INI_GET_PNL_INT(stPnlPara.stTtlCstmInitInfo.u32DlyTmPw2Data, pTargetPnlName, "m_wPanelDlyTm2Data");
            INI_GET_PNL_INT(stPnlPara.stTtlCstmInitInfo.u32DutyVal, pTargetPnlName, "m_wPanelDutyVal");
            INI_GET_PNL_INT(stPnlPara.stTtlCstmInitInfo.u32PeriodVal, pTargetPnlName, "m_wPanelPeriodVal");
            INI_GET_PNL_INT(stPnlPara.stTtlCstmInitInfo.u8PwmNum, pTargetPnlName, "m_wPanelPwmNum");
    ...
    char *szPQpath = NULL;
    szPQpath = iniparser_getstring(d, "LOGO:m_sPQFile0", NULL);
    BOOTLOGO_DBG(BOOTLOGO_DBG_LEVEL_INFO, "Get PQFile file %s!\n", szPQpath);
    if (szPQpath)
    {
        void *pq_fd = inifs_open(szPQpath, O_RDONLY, 0);
        if (pq_fd != NULL)
        {
            stDispOut.u32PQBufferSize = inifs_lseek(pq_fd, 0, SEEK_END);
            BOOTLOGO_DBG(BOOTLOGO_DBG_LEVEL_INFO, "File size 0x%x!\n", stDispOut.u32PQBufferSize);
            stDispOut.pPQbuffer = (char *)malloc(stDispOut.u32PQBufferSize);
            if (stDispOut.pPQbuffer != NULL)
            {
                stDispOut.u32PQBufferSize = inifs_read(pq_fd, stDispOut.pPQbuffer, stDispOut.u32PQBufferSize);
                BOOTLOGO_DBG(BOOTLOGO_DBG_LEVEL_INFO, "Read size 0x%x!\n", stDispOut.u32PQBufferSize);
            }
            inifs_close(pq_fd);
        }
    ...
    if (stDispOut.pPQbuffer)
        free(stDispOut.pPQbuffer);
    return 0;
}
#endif
...
#else
    char  cmd_str[128];
    sprintf(cmd_str, "sf probe 0; sf read 0x%p 0x%p 0x%p", pRawData, start, size);
    run_command(cmd_str, 0);
#endif
