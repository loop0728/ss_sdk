...
rootfs:
	...
	if [ "$(FWFS_MOUNT_BLK)" != "" ]; then \
		...
		mkdir -p $(OUTPUTDIR)/rootfs/misc;\
		$(foreach block, $(FWFS_MOUNT_BLK), $(PROJ_ROOT)/image/makefiletools/script/fwfs_mount.py --part_size=$($(block)$(PATSIZE)) --rcs_dir=$(OUTPUTDIR)/rootfs/etc/init.d/rcS $($(block)$(MOUNTPT)) $($(block)$(MOUNTTG));)\
	fi;
