...
static u32 parse_partition_section(const char *section, u32 start_partion_addr, void *usr_data, u32 *p_i, u8 u8Tunnk, u8 u8BackTrunk, u8 bActive)
{
...
static u32 parse_partition(const char *string, u32 start_partion_addr, void *usr_data, u32 *p_i, u8 u8Tunnk, u8 u8BackTrunk, u8 bActive)
{
    ...
    while(1)
    {
        ...
        if (p_section_end == NULL)
        {
            ...
            start_partion_addr = parse_partition_section(partition_section, start_partion_addr, usr_data, p_i, u8Tunnk, u8BackTrunk, bActive);
            break;
        }
...
int main(int argc, char **argv)
{
    ...
    if (is_write && file_name)
    {
        ...
        if (boot_part0)
            start_partion_addr = parse_partition(boot_part0, start_partion_addr, (void *)&stPni, &i, 0, (boot_part1 != NULL), 1);
        if (boot_part1)
            start_partion_addr = parse_partition(boot_part1, start_partion_addr, (void *)&stPni, &i, 1, 0, 0);
        if (sys_part)
        {
            start_partion_addr = parse_partition(sys_part, start_partion_addr, (void *)&stPni, &i, 0, 0, 1);
        }
